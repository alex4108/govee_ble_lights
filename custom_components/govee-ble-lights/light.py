...
# When set, every GATT session + advert is logged at INFO level with a
# structured "GATT-TRACE" prefix. Useful for correlating Bermuda scanner
# drops with specific phases of a turn_on/turn_off cycle. Flip to True
# (and set the logger to INFO) when diagnosing.
_TRACE = False
...
class GoveeBluetoothLight(LightEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        for i, command in enumerate(other_commands):
            _trace("turn_on %s non-POWER cmd %d/%d CONNECT",
                   self._canonical_mac(), i + 1, len(other_commands))
            client = await self._connectBluetooth()
            try:
                await client.write_gatt_char(UUID_CONTROL_CHARACTERISTIC, command, False)
                _trace("turn_on %s non-POWER cmd %d/%d WROTE bytes=%d",
                       self._canonical_mac(), i + 1, len(other_commands), len(command))
            finally:
                try:
                    await client.disconnect()
                except Exception:
